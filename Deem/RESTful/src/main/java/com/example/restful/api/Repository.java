...
public class Repository {
    ...
    private Repository() {
        ...
        *//*
        ...
        TODO баг с сообщениями. У нас много недодело, например if (!isMyMessage) и растягивание, также как и дизайн картинок, + видео куда то пропадает
        ...
        Итак, вот куча ошибок которые перед нами будут:
        1. Параметра UUID для сообщений нет. Просто обновим в room стараясь не удалять БД.
        2. Поскольку мы не обновляем LiveData когда создаем новое сообщение то оно не сохраняется в кэше и не получается из сервера, в итоге его нет.
        3. Сервер опять же пока еще не настроен на работу с uuid при получении ленты сообщений.
        4. Видео не подгружается если мы отправляем сообщение.
        5. Push сообщений также не содержит uuid пока еще.

        Все вышесказанное но для news, поскольку новости также имеют видео.
        */
